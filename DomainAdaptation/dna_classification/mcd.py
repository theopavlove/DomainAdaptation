...
def main(args: argparse.Namespace):
    ...
    if args.phase == 'test':
        ...
        acc1 = validate(test_loader, G, F1, F2, args, calc_auc=True)
        print(acc1)
        return
...
def validate(val_loader: DataLoader, G: nn.Module, F1: ImageClassifierHead,
             F2: ImageClassifierHead, args: argparse.Namespace, calc_auc: bool = True) -> Tuple[float, float]:
    ...
    with torch.no_grad():
        ...
        if calc_auc:
            y1_preds = torch.cat(y1_preds).cpu().numpy()[:, 1]
            y2_preds = torch.cat(y2_preds).cpu().numpy()[:, 1]
            y_true = torch.cat(y_true).cpu().numpy()
            precision1, recall1, thresholds1 = precision_recall_curve(y_true, y1_preds)
            pr_auc1 = auc(recall1, precision1)
            print(f'F1 PR AUC {pr_auc1 * 100:.3f}')

            precision2, recall2, thresholds2 = precision_recall_curve(y_true, y2_preds)
            pr_auc2 = auc(recall2, precision2)
            print(f'F2 PR AUC {pr_auc2 * 100:.3f}')

            fpr1, tpr1, thresholds1 = roc_curve(y_true, y1_preds)
            roc_auc1 = auc(fpr1, tpr1)
            print(f'F1 ROC AUC {roc_auc1 * 100:.3f}')

            fpr2, tpr2, thresholds2 = roc_curve(y_true, y2_preds)
            roc_auc2 = auc(fpr2, tpr2)
            print(f'F2 ROC AUC {roc_auc2 * 100:.3f}')
